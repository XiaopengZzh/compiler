...
(* If this option is set to true, then we do not actually emit the saving/restoring of
   caller saved registers. This simplifies the interference graph that will be generated
   but leads to obviously incorrect code generation *)
let omit_callee_save = ref false;;

(* operands include scalars (ints), temps (variables), function names
 * (labels), and physical registers.  *)
type operand = Int of int | Var of var | Lab of label | Reg of Riscv.reg
let sp = Reg(Riscv.R2)
let fp = Reg(Riscv.R8)
let ra = Reg(Riscv.R1)
...
| Return  (* return to caller -- result assumed in R10 *)
...
let op2string = function
  ...
  | (Var x) -> x
  | (Reg r) -> "$" ^ Riscv.reg2string r (* prefix registers with $ to disambiguate from vars *)
  | (Lab x) -> x
...
let inst2string i = 
    ...
    | Store(x,i,y) -> "*(" ^ (op2string x) ^ "+" ^ (string_of_int i) ^
                      ") := " ^ (op2string y)
    | Call (x,i) -> "call "^(op2string x)^" " ^ (string_of_int i)^" args"
    | Jump x -> "jump "^x
...
let new_temp() = "t" ^ (string_of_int(inc temp_counter))
...
let fn2blocks (C.Fn {C.name=name;C.args=args;C.body=body;C.pos=pos}) : block list = 
    ...
    let rec emit_exp env (e,pos) : operand = ( 
        ...
        | C.Call(e,es) -> 
            ...
            let arg_space = 4 * (if nargs > 8 then nargs - 8 else 0) in
            ...
            let rec move_args = function
              | (x::xs,i) -> 
                if i < 8 then
                  (emit_inst (Move(Reg(arg_to_reg i), x)); move_args (xs, i+1))
                else
                  (emit_inst (Store(sp,4*(i - 8),x)); move_args(xs,i+1))
              | ([],_) -> () in
    ...
    and emit_stmt env (s,pos) : unit = ( 
        ...
        | C.Return e -> 
            ...
            (* move result into R10 and jump to epilogue *)
            let _ = emit_inst (Move(Reg Riscv.R10,v)) in
    ...
    let callee_regs = 
        [fp;ra] @ (List.map (fun x -> Reg x) 
                      [Riscv.R9;Riscv.R18;Riscv.R19;Riscv.R20;
                       Riscv.R21;Riscv.R22;Riscv.R23;Riscv.R24;Riscv.R25;
                       Riscv.R26;Riscv.R27]) in
    ...
    let rec load_args env plst =
        ...
        | (x::xs,i) ->
            ...
            let _ = (match i with
                      0 -> emit_inst (Move(t,Reg Riscv.R10))
                    | 1 -> emit_inst (Move(t,Reg Riscv.R11))
                    | 2 -> emit_inst (Move(t,Reg Riscv.R12))
                    | 3 -> emit_inst (Move(t,Reg Riscv.R13))
                    | 4 -> emit_inst (Move(t,Reg Riscv.R14))
                    | 5 -> emit_inst (Move(t,Reg Riscv.R15))
                    | 6 -> emit_inst (Move(t,Reg Riscv.R16))
                    | 7 -> emit_inst (Move(t,Reg Riscv.R17))
                    | _ -> emit_inst (Load(t,sp,(i-8)*4))) in
    ...
    let callee_temps = if (not (!omit_callee_save)) then save_callee_regs() else []  in
    ...
    let _ = if (not (!omit_callee_save)) then restore_callee_regs callee_temps else () in
