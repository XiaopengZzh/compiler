...
let i_mem = [Li(R15,0x20l); Sw(R0,R15,0x100l); Lw(R16,R0,0x100l)]
let i_beqt = [Beq(R0,R0,0x8l)]
...
let test_mem mem expected =
  List.map (fun (a,v) -> let v' = read_word_little_endian mem a in
	      if v' = v || v' = (rev_endianess v)
	      then ()
	      else print_string ((format_string "failed (expected,got): " Bright Red)^(tostring v)^", "^(tostring v')^"\n")) expected
...
let test_interpreter () =
  ...
  let _ = testi "i_beqt" i_beqt [] [] (1,0x8l) in
